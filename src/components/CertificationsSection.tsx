...
export const CertificationsSection: React.FC<CertificationsProps> = ({ lang }) => {
  return (
    <section id="certifications" className="py-24 bg-white dark:bg-[#081522] border-t border-slate-200 dark:border-slate-800 transition-colors">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {TEAM_MEMBERS.map((member, idx) => (
              <div
                key={idx}
                className="bg-white dark:bg-[#0B1F3A]/70 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 hover:border-[#0050AE] dark:hover:border-cyan-500/40 flex flex-col justify-between shadow-xs hover:shadow-md transition-all duration-300 group"
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-12 h-12 rounded-full bg-blue-50 dark:bg-[#0B1F3A] border border-blue-200 dark:border-cyan-500/40 flex items-center justify-center text-sm font-bold font-mono text-[#0050AE] dark:text-cyan-300">
                      {member.name.split(' ').map((n) => n[0]).slice(0, 2).join('')}
                    </div>
                    <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-800">
                      MNK Engineer
                    </span>
                  </div>

                  <h4 className="text-base font-bold text-[#002D62] dark:text-white group-hover:text-[#0050AE] dark:group-hover:text-cyan-300 transition-colors">
                    {member.name}
                  </h4>
                ...
                <div className="pt-3 border-t border-slate-100 dark:border-slate-800">
                  <div className="text-[10px] font-mono text-slate-400 mb-1.5 uppercase font-semibold">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {member.certifications.map((cert, cIdx) => (
                      <span
                        key={cIdx}
                        className="text-[10px] font-mono font-semibold px-2 py-0.5 rounded bg-slate-50 dark:bg-slate-900 text-[#0050AE] dark:text-cyan-300 border border-slate-200 dark:border-cyan-500/20"
                      >
                        {cert}
                      </span>
                    ))}
